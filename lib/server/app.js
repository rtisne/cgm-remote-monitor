...
function create (env, ctx) {
  ...
  app.get('/healthz', async (req, res) => {
    try {
      ...
      const mongoOK =
        ctx.store?.client?.topology?.isConnected?.() ?? true; // fallback si pilote change
      if (!mongoOK) throw new Error('mongo disconnected');
  
      // Éventuellement : checker d’autres dépendances ici (Redis, API, etc.)
