...
// Catch SIGTERM/SIGINT so que Kubernetes/Northflank puissent terminer proprement
['SIGTERM', 'SIGINT'].forEach(sig => {
  process.on(sig, () => {
    console.info(`Received ${sig} – shutting down gracefully`);
    // Déclenche le même bus que dans bootevent
    try { require('./bus')().emit('teardown'); } catch (e) { /* noop */ }
    // Fallback : couper au bout de 10 s
